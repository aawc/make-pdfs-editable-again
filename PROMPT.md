...
>     - Create `.github/workflows/release.yml` to concurrently build Windows, Linux, and macOS binaries across AMD64/ARM64 targets automatically on every code commit pushed to the `main` branch. The workflow must dynamically generate a timestamped semantic tag `v$yyyy.$mm.${{ github.run_number }}` using bash date commands and push the tag back to the repository. It must automatically scan all compiled binaries using `crazy-max/ghaction-virustotal@v5`, dynamically calculate each binary's SHA256 hash using `sha256sum` to build a permanent public security analysis report link (`https://www.virustotal.com/gui/file/<sha256>/detection`), attach the binaries to a new GitHub Release with these scan links appended to the release body, and set `generate_release_notes: true` to append the detailed commit history at the bottom. Ensure `cache: false` is configured on all Go setup steps.
